...
/**
 * Send tracking email to user and whitelisted emails
 */
function sendTrackingEmail(PDO $conn, int $order_id) {
    // Fetch order + user + tracking info
    $stmt = $conn->prepare("
    ...
    try {
        ...
        $mail->Password = 'your_email_app_password'; // app password
        $mail->SMTPSecure = 'tls';
        $mail->Port = 587;

        // Sender & recipients
        $mail->setFrom('your_email@gmail.com', 'My Shop');
        ...
        // Email content
        $mail->isHTML(true);
        $mail->Subject = "Order #{$order['order_id']} Tracking Info";
        $mail->Body = "
...
// Example usage: send email for order ID 1
if (sendTrackingEmail($conn, 1)) {
    echo "Tracking email sent!";
} else {
    echo "Failed to send tracking email.";
}
